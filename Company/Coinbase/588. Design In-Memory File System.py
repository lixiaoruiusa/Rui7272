"""
ls 的时间复杂度是 O(m+n+klog(k))。这里 mm 是输入字符串的长度，我们需要扫描输入字符串一次并获得每一层的文件名。n 是最后一层文件夹的深度，我们需要进入 n 层的文件树以到达最后一层文件所在路径。kk是最后一层的文件和文件夹总数目。我们需要将它们排序，所以需要 klog(k)klog(k) 的时间。
...
如果文件夹数目非常大，我们会因为 isfileisfile 和 contentcontent 浪费多余的空间，这部分空间在方法 1 中不会存在。
